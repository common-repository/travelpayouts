<?php
/**
 * Redux_Travelpayouts AJAX Save Class
 *
...
if ( ! class_exists( 'Redux_Travelpayouts_AJAX_Save', false ) ) {

	/**
	 * Class Redux_Travelpayouts_AJAX_Save
	 */
	class Redux_Travelpayouts_AJAX_Save extends Redux_Travelpayouts_Class {

		/**
		 * Redux_Travelpayouts_AJAX_Save constructor.
		...
		public function save() {
			...
			if ( ! Redux_Travelpayouts_Helpers::current_user_can( $core->args['page_permissions'] ) ) {
				echo wp_json_encode(
					array(
			...
			if ( isset( $_POST['opt_name'] ) && ! empty( $_POST['opt_name'] ) && isset( $_POST['data'] ) && ! empty( $_POST['data'] ) ) {
				$redux = Redux_Travelpayouts::instance( sanitize_text_field( wp_unslash( $_POST['opt_name'] ) ) );
				...
				if ( ! empty( $redux->args['opt_name'] ) ) {
					...
					$values = Redux_Travelpayouts_Functions_Ex::parse_str( $post_data );
			...
			if ( isset( $return_array ) ) {
				...
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				echo wp_json_encode( apply_filters( 'redux_travelpayouts/options/' . $core->args['opt_name'] . '/ajax_save/response', $return_array ) );
			}
