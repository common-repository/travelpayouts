<?php
/**
 * Redux_Travelpayouts Page Render Class
 *
 * @class Redux_Travelpayouts_Page_Render
 * @version 3.0.0
...
if ( ! class_exists( 'Redux_Travelpayouts_Page_Render', false ) ) {

	/**
	 * Class Redux_Travelpayouts_Page_Render
	 */
	class Redux_Travelpayouts_Page_Render extends Redux_Travelpayouts_Class {
		...
		/**
		 * Redux_Travelpayouts_Page_Render constructor.
		 *
			...
			if ( 'hidden' === $core->args['menu_type'] ) {
				...
				if ( true === $core->args['allow_sub_menu'] ) {
					foreach ( $core->sections as $k => $section ) {
						...
						if ( isset( $section['permissions'] ) && ! Redux_Travelpayouts_Helpers::current_user_can( $section['permissions'] ) ) {
							continue;
						}
		...
		public function load_page() {
			...
			if ( true === $this->show_hints ) {
				...
				// Make URL.
				$nonce = wp_create_nonce( 'Redux_Travelpayouts_hint_toggle' );
				$url   = '<a class="Redux_Travelpayouts_hint_status" href="?nonce=' . $nonce . '&amp;dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . esc_attr( $cur_page ) . '&amp;tab=' . esc_attr( $cur_tab ) . '">' . $s . ' hints</a>';
				...
				$msg = sprintf( esc_html__( 'Hints are tooltips that popup when %1$s the hint icon, offering addition information about the field in which they appear.  They can be %2$s by using the link below.', 'redux-framework' ), $event, Redux_Travelpayouts_Core::strtolower( $s ) ) . '<br/><br/>' . $url;
			...
			if ( '' !== $core->args['help_sidebar'] ) {
				...
				// about hints.
				if ( true === $this->show_hints ) {
					$screen->set_help_sidebar( '<p><strong>Redux_Travelpayouts Framework</strong><br/><br/>' . esc_html__( 'Hint Tooltip Preferences', 'redux-framework' ) . '</p>' );
				}
			}

			/**
			 * Action 'redux_travelpayouts/page/{opt_name}/load'
			 *
			 * @param object $screen WP_Screen
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( "redux_travelpayouts/page/{$core->args['opt_name']}/load", $screen );
		}
		...
		private function submenu( $core ) {
			...
			$page_parent = Redux_Travelpayouts_Core::strtolower( $page_parent );
		...
		public function generate_panel() {
			...
			$panel = new Redux_Travelpayouts_Panel( $core );
			$panel->init();
		...
		public function field_input( $field, $v = null, $metabox = false ) {
			...
			if ( isset( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
				...
				/**
				 * Action 'redux_travelpayouts/field/{opt_name}/{field.type}/callback/before'
				 *
				...
				do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"redux_travelpayouts/field/{$core->args['opt_name']}/{$field['type']}/callback/before",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'redux_travelpayouts/field/{opt_name}/callback/before'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"redux_travelpayouts/field/{$core->args['opt_name']}/callback/before",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'redux_travelpayouts/field/{opt_name}/{field.type}/callback/after'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"redux_travelpayouts/field/{$core->args['opt_name']}/{$field['type']}/callback/after",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'redux_travelpayouts/field/{opt_name}/callback/after'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"redux_travelpayouts/field/{$core->args['opt_name']}/callback/after",
					array(
						&$field,
						&$value,
					)
			...
			if ( isset( $field['type'] ) ) {
				...
				if ( Redux_Travelpayouts_Core::$pro_loaded ) {
					$pro_path = Redux_Travelpayouts_Pro::$dir . "core/inc/fields/{$field['type']}/class-travelpayouts-pro-{$field_type}.php";
				...
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$class_file = apply_filters( "redux_travelpayouts/{$core->args['opt_name']}/field/class/{$field['type']}", $filter_path, $field );
				...
				if ( class_exists( $field_class ) ) {
					...
					/**
					 * Action 'redux_travelpayouts/field/{opt_name}/{field.type}/render/before'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux_travelpayouts/field/{$core->args['opt_name']}/{$field['type']}/render/before",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Action 'redux_travelpayouts/field/{$this->args['opt_name']}/render/before'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux_travelpayouts/field/{$core->args['opt_name']}/render/before",
						array(
							&$field,
							&$value,
						)
					...
					$pro_field_loaded = Redux_Travelpayouts_Functions::load_pro_field( $data );
					...
					/**
					 * Filter 'redux_travelpayouts/field/{opt_name}'
					 *
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "redux_travelpayouts/field/{$core->args['opt_name']}", ob_get_contents(), $field );

					/**
					 * Filter 'redux_travelpayouts/field/{opt_name}/{field.type}/render/after'
					 *
					 * @param       string        rendered field markup
					 * @param array $field field data
					 */
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "redux_travelpayouts/field/{$core->args['opt_name']}/{$field['type']}/render/after", $_render, $field );

					/**
					 * Filter 'redux_travelpayouts/field/{opt_name}/render/after'
					 *
					 * @param       string        rendered field markup
					 * @param array $field field data
					 */
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "redux_travelpayouts/field/{$core->args['opt_name']}/render/after", $_render, $field );
					...
					/**
					 * Action 'redux_travelpayouts/field/{opt_name}/{field.type}/fieldset/before/{opt_name}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux_travelpayouts/field/{$core->args['opt_name']}/{$field['type']}/fieldset/before/{$core->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Action 'redux_travelpayouts/field/{opt_name}/fieldset/before/{opt_name}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux_travelpayouts/field/{$core->args['opt_name']}/fieldset/before/{$core->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
					...
					if ( isset( $field['full_width'] ) && true === $field['full_width'] ) {
						$class_string .= 'Redux_Travelpayouts_remove_th';
					}
                    ...
                    $wrapField = $field['wrapField'] ?? true;
                    if ($wrapField) {
                        echo '<fieldset id="' . esc_attr($core->args['opt_name'] . '-' . $field['id']) . '" class="' . esc_attr($hidden . esc_attr($disabled) . 'redux-field-container tp-admin-field-container__wrapper redux-field redux-field-init redux-container-' . $field['type'] . ' ' . $class_string) . '" data-id="' . esc_attr($field['id']) . '" data-type="' . esc_attr($field['type']) . '">';
                        echo '<div class="tp-admin-field-container"><div class="tp-admin-field-container__content">';
                    }
                    ...
                    if ($wrapField) {
                        echo '</div>';
                        echo (isset($field['description']) && 'info' !== $field['type'] && 'section' !== $field['type'] && !empty($field['description'])) ? '<div class="description field-desc tp-admin-field-container__description">' . wp_kses_post($field['description']) . '</div>' : '';
                        echo '</div>';
                        echo '</fieldset>';
                    }

                    /**
                     * Action 'redux_travelpayouts/field/{opt_name}/{field.type}/fieldset/after/{opt_name}'
                     * @param array $field field data
                     * @param string $value field id
                     */
                    // phpcs:ignore WordPress.NamingConventions.ValidHookName
                    do_action_ref_array(
                    // phpcs:ignore WordPress.NamingConventions.ValidHookName
                        "redux_travelpayouts/field/{$core->args['opt_name']}/{$field['type']}/fieldset/after/{$core->args['opt_name']}",
                        [
                            &$field,
                            &$value,
                        ]
                    ...
                    /**
                     * Action 'redux_travelpayouts/field/{opt_name}/fieldset/after/{opt_name}'
                     *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"redux_travelpayouts/field/{$core->args['opt_name']}/fieldset/after/{$core->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
		...
		public function add_menu() {
			...
			if ( $menu ) {
				...
				if ( isset( $submenu[ $core->args['page_slug'] ] ) && is_array( $submenu[ $core->args['page_slug'] ] ) ) {
					foreach ( $submenu[ $core->args['page_slug'] ] as $index => $Redux_Travelpayouts_options_submenu ) {
						$subnodeargs = array(
							'id'     => esc_html( $core->args['page_slug'] . '_' . $index ),
							'title'  => esc_html( $Redux_Travelpayouts_options_submenu[0] ),
							'parent' => esc_html( $core->args['page_slug'] ),
							'href'   => esc_url( admin_url( 'admin.php?page=' . $Redux_Travelpayouts_options_submenu[2] ) ),
						);
		...
		public function admin_head() {
			...
			/**
			 * Action 'redux_travelpayouts/page/{opt_name}/header'
			 *
			 * @param  object $this TravelpayoutsSettingsFramework
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( "redux_travelpayouts/page/{$core->args['opt_name']}/header", $core );
		}
		...
		public function get_header_html( $field ) {
			...
			if ( isset( $field['title'] ) && isset( $field['type'] ) && 'info' !== $field['type'] && 'section' !== $field['type'] ) {
				...
				if ( isset( $field['hint'] ) && ! empty( $field['hint'] ) ) {
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$hint = apply_filters( 'redux_travelpayouts/hints/html', $hint, $field, $core->args );
					...
					if ( 'true' === $meta_val || empty( $meta_val ) && empty( $hint ) ) {
						...
						$hint = '<div class="redux-hint-qtip" style="float:' . esc_attr( $core->args['hints']['icon_position'] ) . '; font-size: ' . esc_attr( $size ) . '; color:' . esc_attr( $hint_color ) . '; cursor: ' . $pointer . ';" qtip-title="' . esc_attr( $title_param ) . '" qtip-content="' . wp_kses_post( $content_param ) . '">&nbsp;<i class="tp-admin-sidebar-menu-item__icon ' . ( isset( $core->args['hints']['icon'] ) ? esc_attr( $core->args['hints']['icon'] ) : '' ) . '"></i></div>';
					}
				}

				if ( ! empty( $field['title'] ) ) {
				    $th = '<div class="tp-field-title__label">';
				...
				if ( isset( $field['subtitle'] ) && is_string($field['subtitle']) && !empty($field['subtitle']) ) {
					$th .= '<span class="description tp-field-title__description">' . wp_kses_post( $field['subtitle'] ) . '</span>';
				}
			}

			if ( ! empty( $th ) ) {
				$th = '<div class="redux_field_th tp-field-title">' . $th . '</div>';
			}
		...
		public function section_menu( $k, $section, $suffix = '', $sections = array() ) {
			...
			if ( ( ( isset( $core->args['icon_type'] ) && 'image' === $core->args['icon_type'] ) || ( isset( $section['icon_type'] ) && 'image' === $section['icon_type'] ) ) || ( isset( $section['icon'] ) && false !== strpos( $section['icon'], '/' ) ) ) {
				...
				$icon = ( ! isset( $section['icon'] ) ) ? '<i class="tp-admin-sidebar-menu-item__icon el el-cog' . esc_attr( $icon_class ) . '"></i> ' : '<i class="tp-admin-sidebar-menu-item__icon ' . esc_attr( $section['icon'] ) . esc_attr( $icon_class ) . '"></i> ';
			}
			...
			if ( isset( $section['type'] ) && 'divide' === $section['type'] ) {
				...
				$string            .= '<li id="' . esc_attr( $k . $suffix ) . '_section_group_li" class="tp-admin-sidebar-menu-item redux-group-tab-link-li' . esc_attr( $hide_section ) . esc_attr( $section['class'] ) . esc_attr( $subsections_class ) . '">';
				$string            .= '<a href="javascript:void(0);" id="' . esc_attr( $k . $suffix ) . '_section_group_li_a" class="tp-admin-sidebar-menu-item__link redux-group-tab-link-a" data-key="' . esc_attr( $k ) . '" data-rel="' . esc_attr( $k . $suffix ) . '" data-section-id="'.$section['id'].'">' . $extra_icon . $icon . '<span class="tp-admin-sidebar-menu-item__label group_title">' . wp_kses_post( $section['title'] ) . '</span></a>';
				...
				if ( $subsections ) {
					...
					$string .= '<ul id="' . esc_attr( $next_k . $suffix ) . '_section_group_li_subsections" class="tp-admin-sidebar-menu-subsection subsection">';
					$do_loop = true;
					...
					while ( $do_loop ) {
						...
						if ( count( $sections ) < $next_k || ! isset( $sections[ $next_k ] ) || ! isset( $sections[ $next_k ]['subsection'] ) || true !== $sections[ $next_k ]['subsection'] ) {
							...
							if ( ( isset( $core->args['icon_type'] ) && 'image' === $core->args['icon_type'] ) || ( isset( $sections[ $next_k ]['icon_type'] ) && 'image' === $sections[ $next_k ]['icon_type'] ) ) {
								$icon = ( ! isset( $sections[ $next_k ]['icon'] ) ) ? '' : '<img class="tp-admin-sidebar-menu-item__icon image_icon_type" src="' . esc_url( $sections[ $next_k ]['icon'] ) . '" /> ';
							} else {
								...
								$icon = ( ! isset( $sections[ $next_k ]['icon'] ) ) ? '' : '<i class="tp-admin-sidebar-menu-item__icon ' . esc_attr( $sections[ $next_k ]['icon'] ) . esc_attr( $icon_class ) . '"></i> ';
							}
							if ( strpos( $icon, 'el-icon-' ) !== false ) {
								$icon = str_replace( 'el-icon-', 'el el-', $icon );
							}
							...
							$string                      .= '<li id="' . esc_attr( $next_k . $suffix ) . '_section_group_li" class="tp-admin-sidebar-menu-item redux-group-tab-link-li ' . esc_attr( $hide_sub ) . esc_attr( $section[ $next_k ]['class'] ) . ( $icon ? ' hasIcon tp-admin-sidebar-menu-item--with-icon' : '' ) . '">';
							$string                      .= '<a href="javascript:void(0);" id="' . esc_attr( $next_k . $suffix ) . '_section_group_li_a" class="redux-group-tab-link-a tp-admin-sidebar-menu-item__link" data-key="' . esc_attr( $next_k ) . '" data-rel="' . esc_attr( $next_k . $suffix ) . '" data-section-id="'.$sections[ $next_k ]['id'].'">' . $icon . '<span class="tp-admin-sidebar-menu-item__label group_title">' . wp_kses_post( $sections[ $next_k ]['title'] ) . '</span></a>';
							$string                      .= '</li>';
