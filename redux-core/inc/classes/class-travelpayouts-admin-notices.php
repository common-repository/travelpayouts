<?php
/**
 * Redux_Travelpayouts Framework Admin Notice Class
...
// Don't duplicate me!
if ( ! class_exists( 'Redux_Travelpayouts_Admin_Notices', false ) ) {

	/**
	 * Redux_Travelpayouts Admin Notices Class
	 *
	 * @since       3.0.0
	 */
	class Redux_Travelpayouts_Admin_Notices extends Redux_Travelpayouts_Class {
		...
		/**
		 * Redux_Travelpayouts_Admin_Notices constructor.
		 *
		 * @param array $parent TravelpayoutsSettingsFramework object.
		 * @access public
		 */
		public function __construct( $parent ) {
			parent::__construct( $parent );

			add_action( 'wp_ajax_Redux_Travelpayouts_hide_admin_notice', array( $this, 'ajax' ) );
		...
		public function admin_notices( $notices = array() ) {
			...
			if ( isset( $_GET ) && isset( $_GET['page'] ) && $core->args['page_slug'] === $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification
				do_action( 'Redux_Travelpayouts_admin_notices_run', $core->args );
								...
								jQuery( document ).ready( function( $ ) {
									$( document.body ).on(
										'click', '.redux-notice.is-dismissible .notice-dismiss', function( e ) {
											...
											$.post(
												ajaxurl, {
													action: 'Redux_Travelpayouts_hide_admin_notice',
													id: $data.attr( 'id' ),
													nonce: $data.val()
												}
											);
		...
		private function dismiss_admin_notice() {
			...
			if ( isset( $_GET['dismiss'] ) && isset( $_GET['id'] ) ) {
				if ( isset( $_GET['nonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_GET['nonce'] ) ), 'Redux_Travelpayouts_hint_toggle' ) ) {
					...
					wp_nonce_ays( 'Redux_Travelpayouts_hint_toggle' );
				}
			}
		}
		...
		public function ajax() {
			...
			if ( isset( $_POST['id'] ) ) {
                ...
                if (!isset($_REQUEST['nonce']) || !wp_verify_nonce(sanitize_key(wp_unslash($_POST['nonce'])), $id . $userid . 'nonce')) {
					die( 0 );
				} else {
					// Add the dismiss request to the user meta.
