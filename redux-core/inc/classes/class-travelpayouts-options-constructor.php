<?php
/**
 * Redux_Travelpayouts Options Class
 *
 * @class Redux_Travelpayouts_Options
 * @version 3.0.0
...
if ( ! class_exists( 'Redux_Travelpayouts_Options_Object', false ) ) {
	 ...
	 * Class Redux_Travelpayouts_Options
	 */
	class Redux_Travelpayouts_Options_Constructor extends Redux_Travelpayouts_Class {
		...
		/**
		 * Redux_Travelpayouts_Options constructor.
		 *
			...
			foreach ( $sections as $key => $section ) {
				if ( 'fields' === $key ) {
					foreach ( $section as $field ) {
						if ( ! empty( $field['id'] ) && ! empty( $field['data'] ) && ! empty( $options_values[ $field['id'] ] ) && \Redux_Travelpayouts_Helpers::is_integer( $options_values[ $field['id'] ] ) ) {
		...
		public function get() {
			...
			/**
			 * Action 'redux_travelpayouts/options/{opt_name}/options'
			 *
			 * @param mixed $value option values
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$core->options = apply_filters( "redux_travelpayouts/options/{$core->args['opt_name']}/options", $core->options, $core->sections );
		 ...
		 *
		 * @since TravelpayoutsSettingsFramework 3.0.0
		 *
		 ...
		 * Set a global variable by the global_variable argument
		 *
		 * @param object $core TravelpayoutsSettingsFramework core object.
		 *
		 * @since   3.1.5
		 *
		...
		private function set_global_variable( $core ) {
			if ( ! empty( $core->args['global_variable'] ) ) {
				...
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$GLOBALS[ $options_global ] = apply_filters( "redux_travelpayouts/options/{$core->args['opt_name']}/global_variable", $core->options );

				// Last save key.
				if ( isset( $core->transients['last_save'] ) ) {
					$GLOBALS[ $options_global ]['Redux_Travelpayouts_LAST_SAVE'] = $core->transients['last_save'];
				}

				// Last compiler hook key.
				if ( isset( $core->transients['last_compiler'] ) ) {
					$GLOBALS[ $options_global ]['Redux_Travelpayouts_LAST_COMPILER'] = $core->transients['last_compiler'];
				}
		...
		public function register() {
			...
			foreach ( $core->sections as $k => $section ) {
				...
				/**
				 * Filter 'redux_travelpayouts/options/{opt_name}/section/{section.id}'
				 *
				 * @param array $section section configuration
				 */
				if ( isset( $section['id'] ) ) {
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$section = apply_filters( "redux_travelpayouts/options/{$core->args['opt_name']}/section/{$section['id']}", $section );
				}
				...
				if ( isset( $section['permissions'] ) && false !== $section['permissions'] ) {
					if ( ! Redux_Travelpayouts_Helpers::current_user_can( $section['permissions'] ) ) {
				...
				if ( isset( $section['fields'] ) ) {
					foreach ( $section['fields'] as $fieldk => $field ) {
						...
						/**
						 * Filter 'redux_travelpayouts/options/{opt_name}/field/{field.id}'
						 *
						 * @param array $field field config
						 */

						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						$field = apply_filters( "redux_travelpayouts/options/{$core->args['opt_name']}/field/{$field['id']}/register", $field );
						...
						if ( isset( $field['permissions'] ) && false !== $field['permissions'] ) {
							if ( ! Redux_Travelpayouts_Helpers::current_user_can( $field['permissions'] ) ) {
                        ...
                        /**
                         * Игнорируем поля, для которых установлен параметр skipSave
                         */
                        if(isset($field['skipSave']) && $field['skipSave'] === true) {
                            $do_update = false;
                        }
						...
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						$field = apply_filters( "redux_travelpayouts/options/{$core->args['opt_name']}/field/{$field['id']}", $field );
						...
						/**
						 * Action 'redux_travelpayouts/options/{opt_name}/field/field.type}/register'
						 */

						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						do_action( "redux_travelpayouts/options/{$core->args['opt_name']}/field/{$field['type']}/register", $field );
						...
						if ( ! $display || isset( $this->no_panel_section[ $k ] ) ) {
							...
							if ( isset( $field['disabled'] ) && $field['disabled'] ) {
								$field['label_for'] = 'Redux_Travelpayouts_disable_field';
							}

							if ( isset( $field['hidden'] ) && $field['hidden'] ) {
								$field['label_for'] = 'Redux_Travelpayouts_hide_field';
							}
			...
			/**
			 * Action 'redux_travelpayouts/options/{opt_name}/register'
			 *
			 * @param array option sections
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( "redux_travelpayouts/options/{$core->args['opt_name']}/register", $core->sections );
			...
			if ( isset( $core->transients['run_compiler'] ) && $core->transients['run_compiler'] ) {
				...
				/**
				 * Action 'redux_travelpayouts/options/{opt_name}/compiler'
				 *
				 * @param array  options
				...
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				do_action( "redux_travelpayouts/options/{$core->args['opt_name']}/compiler", $core->options, $compiler_css, $core->transients['changed_values'], $core->output_variables );

				/**
				 * Action 'redux_travelpayouts/options/{opt_name}/compiler/advanced'
				 *
				 * @param array  options
				 * @param string CSS that get sent to the compiler hook, which sends the full Redux_Travelpayouts object
				 */

				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				do_action( "redux_travelpayouts/options/{$core->args['opt_name']}/compiler/advanced", $core );
		...
		public function default_values() {
			...
			/**
			 * Filter 'redux_travelpayouts/options/{opt_name}/defaults'
			 *
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$core->options_defaults = apply_filters( "redux_travelpayouts/options/{$core->args['opt_name']}/defaults", $core->options_defaults, $core->transients['changed_values'] );
		...
		public function validate_options( $plugin_options ) {
			...
			if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
				...
				if ( ! empty( $imported_options ) && is_array( $imported_options ) && isset( $imported_options['redux-backup'] ) && 1 === $imported_options['redux-backup'] ) {
					 ...
					 *
					 * @param  &array [&$plugin_options, Redux_Travelpayouts_options]
					 */

					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					do_action_ref_array(
						"redux_travelpayouts/options/{$core->args['opt_name']}/import", // phpcs:ignore WordPress.NamingConventions.ValidHookName
						array(
							&$plugin_options,
							$imported_options,
					...
					setcookie( 'Redux_Travelpayouts_current_tab_' . $core->args['opt_name'], '', 1, '/', $time + 1000, '/' );
					$_COOKIE[ 'Redux_Travelpayouts_current_tab_' . $core->args['opt_name'] ] = 1;
			...
			if ( ! empty( $plugin_options['defaults'] ) ) {
				...
				/**
				 * Filter: 'redux_travelpayouts/validate/{opt_name}/defaults'.
				 *
				 * @param  &array [ $this->options_defaults, $plugin_options]
				 */

				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$plugin_options = apply_filters( "redux_travelpayouts/validate/{$core->args['opt_name']}/defaults", $core->options_defaults );
			...
			if ( ! empty( $plugin_options['defaults-section'] ) ) {
				if ( isset( $plugin_options['redux-section'] ) && isset( $core->sections[ $plugin_options['redux-section'] ]['fields'] ) ) {
					...
					/**
					 * Filter: 'redux_travelpayouts/validate/{opt_name}/defaults_section'.
					 *
					 * @param  &array [ $this->options_defaults, $plugin_options]
					 */

					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$plugin_options = apply_filters( "redux_travelpayouts/validate/{$core->args['opt_name']}/defaults_section", $plugin_options );
				}
			...
			/**
			 * Filter: 'redux_travelpayouts/validate/{opt_name}/before_validation'
			 *
			 * @param  &array [&$plugin_options, Redux_Travelpayouts_options]
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$plugin_options = apply_filters( "redux_travelpayouts/validate/{$core->args['opt_name']}/before_validation", $plugin_options, $core->options );
			...
			/**
			 * Action 'redux_travelpayouts/options/{opt_name}/validate'
			 *
			 * @param  &array [&$plugin_options, Redux_Travelpayouts_options]
			 */
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action_ref_array(
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
				"redux_travelpayouts/options/{$core->args['opt_name']}/validate",
				array(
					&$plugin_options,
					$core->options,
					$core->transients['changed_values'],
