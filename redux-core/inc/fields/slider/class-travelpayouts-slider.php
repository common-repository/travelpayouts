...
if ( ! class_exists( 'Redux_Travelpayouts_Slider', false ) ) {

	/**
	 * Class Redux_Travelpayouts_Slider
	 */
	class Redux_Travelpayouts_Slider extends Redux_Travelpayouts_Field {
		 ...
		 *
		 * @since Redux_Travelpayouts_Framework 3.1.8
		 */
		...
		public function enqueue() {
			...
			wp_enqueue_style(
				...
				Redux_Travelpayouts_Core::$url . "assets/css/vendor/nouislider$min.css",
				array(),
				'5.0.0',
				'all'
			);
			...
			wp_register_script(
				'redux-nouislider-js',
				Redux_Travelpayouts_Core::$url . 'assets/js/vendor/nouislider/redux.jquery.nouislider' . $min . '.js',
				array( 'jquery' ),
				'5.0.0',
				true
			);
			...
			wp_enqueue_script(
				...
				Redux_Travelpayouts_Core::$url . 'inc/fields/slider/redux-slider' . $min . '.js',
			...
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					...
					Redux_Travelpayouts_Core::$url . 'inc/fields/slider/redux-slider.css',
					array(),
					$this->timestamp,
					'all'
				);
		 ...
		 *
		 * @since TravelpayoutsSettingsFramework 0.0.4
		 */
		public function render() {
			...
			if ( $this->display_text === $this->field['display_value'] ) {
				...
				echo '<input 
                        ...
                        class="redux-slider-input tp-input redux-slider-input-one-' . esc_attr( $field_id ) . ' ' . esc_attr( $this->field['class'] ) . '"/>';
			...
			// DIV output.
            echo '<div
	                class="redux-slider-container tp-slider ' . ($two_handles ? 'tp-slider--range' : 'tp-slider--single' ). esc_attr($this->field['class']) . '"
			...
			if ( true === $two_handles ) {
				...
				if ( true === $show_input ) {
					echo '<input 
                            ...
                            class="redux-slider-input tp-input redux-slider-input-two-' . esc_attr( $field_id ) . ' ' . esc_attr( $this->field['class'] ) . '"/>';
				}
...
class_alias( 'Redux_Travelpayouts_Slider', 'TravelpayoutsSettingsFramework_Slider' );
