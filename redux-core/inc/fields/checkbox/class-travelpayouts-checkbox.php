...
// Don't duplicate me!
if ( ! class_exists( 'Redux_Travelpayouts_Checkbox', false ) ) {

	/**
	 * Main Redux_Travelpayouts_checkbox class
	 *
	 * @since       1.0.0
	 */
	class Redux_Travelpayouts_Checkbox extends Redux_Travelpayouts_Field {
		...
		public function render() {
			...
			if ( ! empty( $this->field['options'] ) && ( is_array( $this->field['options'] ) || is_array( $this->field['default'] ) ) ) {
				...
				foreach ( $this->field['options'] as $k => $v ) {
					...
					echo '<label class="tp-checkbox-wrapper" for="' . esc_attr( $id ) . '">';
					...
					echo '<input type="checkbox" class="checkbox tp-input tp-checkbox ' . esc_attr( $this->field['class'] ) . '" id="' . esc_attr( $id ) . '" value="1" ' . checked( $this->value[ $k ], '1', false ) . '/>';
					echo '<div class="tp-checkbox-label">';
					echo ' ' . esc_attr( $v ) . '</div></label>';
					echo '</li>';
				}
				...
				if ( ! empty( $this->field['label'] ) ) {
					echo '<label class="tp-align-items-center tp-d-flex">';
				}
		...
		public function enqueue() {
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					...
					Redux_Travelpayouts_Core::$url . 'inc/fields/checkbox/redux-checkbox.css',
					array(),
					$this->timestamp,
					'all'
				);
			...
			wp_enqueue_script(
				...
				Redux_Travelpayouts_Core::$url . 'inc/fields/checkbox/redux-checkbox' . Redux_Travelpayouts_Functions::is_min() . '.js',
...
class_alias( 'Redux_Travelpayouts_Checkbox', 'TravelpayoutsSettingsFramework_Checkbox' );
