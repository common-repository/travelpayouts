...
if ( ! class_exists( 'Redux_Travelpayouts_Dimensions', false ) ) {

	/**
	 * Class Redux_Travelpayouts_Dimensions
	 */
	class Redux_Travelpayouts_Dimensions extends Redux_Travelpayouts_Field {
		...
		public function render() {
			...
			$arr_units = Redux_Travelpayouts_Helpers::$array_units;

			$unit_check   = $arr_units;
			$unit_check[] = false;

			// If units field has a value but is not an acceptable value, unset the variable.
			if ( isset( $this->field['units'] ) && ! Redux_Travelpayouts_Helpers::array_in_array( $this->field['units'], $unit_check ) ) {
				unset( $this->field['units'] );
			}

			// If there is a default unit value  but is not an accepted value, unset the variable.
			if ( isset( $this->value['units'] ) && ! Redux_Travelpayouts_Helpers::array_in_array( $this->value['units'], $unit_check ) ) {
			...
			echo '<div id="' . esc_attr( $this->field['id'] ) . '-fieldset" class="redux-dimensions-container" data-id="' . esc_attr( $this->field['id'] ) . '">';
		...
		public function enqueue() {
			...
			wp_enqueue_script(
				...
				Redux_Travelpayouts_Core::$url . 'inc/fields/dimensions/redux-dimensions' . Redux_Travelpayouts_Functions::is_min() . '.js',
			...
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					...
					Redux_Travelpayouts_Core::$url . 'inc/fields/dimensions/redux-dimensions.css',
					array(),
					$this->timestamp,
					'all'
				);
...
class_alias( 'Redux_Travelpayouts_Dimensions', 'TravelpayoutsSettingsFramework_Dimensions' );
