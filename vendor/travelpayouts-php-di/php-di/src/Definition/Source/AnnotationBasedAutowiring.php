...
use Travelpayouts\Vendor\DI\Annotation\Injectable;
use Travelpayouts\Vendor\DI\Definition\Exception\InvalidAnnotation;
...
use Travelpayouts\Vendor\DI\Definition\ObjectDefinition\PropertyInjection;
use Travelpayouts\Vendor\DI\Definition\Reference;
use Travelpayouts\Vendor\Doctrine\Common\Annotations\AnnotationRegistry;
use Travelpayouts\Vendor\Doctrine\Common\Annotations\Reader;
use Travelpayouts\Vendor\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use Travelpayouts\Vendor\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'Travelpayouts\Vendor\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'Travelpayouts\Vendor\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('Travelpayouts\Vendor\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()
                ->getClassAnnotation($class, 'Travelpayouts\Vendor\DI\Annotation\Injectable');
