...
class CompareValidator extends Validator
{
     ...
     * - [[TYPE_STRING|string]]: the values are being compared as strings. No conversion will be done before
     *     comparison.
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            switch ($this->operator) {
                case '===':
                case '==':
                    $this->message = Travelpayouts::_x('"{attribute}" must be equal to "{compareValueOrAttribute}".', 'validator.compareValidator');
                    break;
                case '!==':
                case '!=':
                    $this->message = Travelpayouts::_x('"{attribute}" must not be equal to "{compareValueOrAttribute}".', 'validator.compareValidator');
                    break;
                case '>':
                    $this->message = Travelpayouts::_x('"{attribute}" must be greater than "{compareValueOrAttribute}".', 'validator.compareValidator');
                    break;
                case '>=':
                    $this->message = Travelpayouts::_x('"{attribute}" must be greater than or equal to "{compareValueOrAttribute}".', 'validator.compareValidator');
                    break;
                case '<':
                    $this->message = Travelpayouts::_x('"{attribute}" must be less than "{compareValueOrAttribute}".', 'validator.compareValidator');
                    break;
                case '<=':
                    $this->message = Travelpayouts::_x('"{attribute}" must be less than or equal to "{compareValueOrAttribute}".', 'validator.compareValidator');
                    break;
                default:
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        if (is_array($value)) {
            $this->addError($model, $attribute, Travelpayouts::_x('"{attribute}" is invalid.', 'validator.compareValidator'));
