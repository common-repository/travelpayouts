...
class FileCache extends Cache
{
    ...
    public function init()
    {
        parent::init();
        try {
            $this->cachePath = Travelpayouts::getAlias($this->cachePath);
    ...
    protected function setValue($key, $value, $duration)
    {
        ...
        Logs::write([
            "Unable to write cache file '{$cacheFile}': {$error['message']}",
            __METHOD__,
        ]);
        return false;
    }
                ...
                if (is_dir($fullPath)) {
                    ...
                    if (!$expiredOnly) {
                        if (!@rmdir($fullPath)) {
                            $error = error_get_last();
                            Logs::write([
                                "Unable to remove directory '{$fullPath}': {$error['message']}",
                                __METHOD__,
                            ]);
                        }
                    }
                    ...
                    if (!@unlink($fullPath)) {
                        $error = error_get_last();
                        Logs::write([
                            "Unable to remove file '{$fullPath}': {$error['message']}",
                            __METHOD__,
                        ]);
                    }
                }
